...
import { gatherJupIntel, gatherPolyIntel, type MarketIntel } from "./market-intel";
...
function llmKnowledgeBonus(question: string): number {
  ...
  if (
    /\b(peru|peruvian|hungary|hungarian|eurovision|bolivia|ecuador|colombia|paraguay|chile|brazil|mexico|kenya|nigeria|philippines|indonesia)\b/.test(
      q,
    )
  )
    return -0.5;
  ...
  if (
    /\b(apple|google|meta|microsoft|openai|nvidia|tesla|spacex|ai |artificial intelligence|iphone|android)\b/.test(
      q,
    )
  )
    return 0.9;
  ...
  if (
    /\b(nba|nfl|mlb|nhl|premier league|champions league|world cup|super bowl|stanley cup|world series|formula 1|f1 |ufc |boxing)\b/.test(
      q,
    )
  )
    return 0.8;
  ...
  if (
    /\b(trump|biden|desantis|newsom|harris|democrat|republican).*(president|election|2028|2026)/.test(
      q,
    )
  )
    return 0.7;
...
export async function scanPolymarketMarkets(
  ...
  const [samplingRes, gammaVolRes, gammaEndRes] = await Promise.allSettled([
    withRetry(() => fetch("https://clob.polymarket.com/sampling-markets"), {
      label: "poly-sampling",
    }),
    withRetry(
      () =>
        fetch(
          "https://gamma-api.polymarket.com/markets?closed=false&active=true&order=volume24hr&ascending=false&limit=300",
        ),
    ...
    withRetry(
      () =>
        fetch(
          "https://gamma-api.polymarket.com/markets?closed=false&active=true&order=end_date&ascending=true&limit=300",
        ),
  ...
  for (const gammaRes of gammaSources) {
    ...
    for (const g of Array.isArray(gammaData) ? gammaData : []) {
      ...
      try {
        ...
        outcomes = String(g.outcomes ?? "").split(",");
      }
      try {
        prices = JSON.parse(String(g.outcomePrices ?? "[]"));
      } catch {
        prices = String(g.outcomePrices ?? "").split(",");
      }
      try {
        ids = JSON.parse(String(g.clobTokenIds ?? g.clob_token_ids ?? "[]"));
      } catch {
        ids = String(g.clobTokenIds ?? g.clob_token_ids ?? "").split(",");
      }
  ...
  for (const m of rawMarkets) {
    ...
    if (yp < POLY_PRICE_MIN || yp > POLY_PRICE_MAX) {
      skipPrice++;
      continue;
    }
    const q = String(m.question ?? "");
    if (ownedTitles.has(q.toLowerCase())) {
      skipOwned++;
      continue;
    }
    if (isRecentlyTraded(state, q)) {
      skipRecent++;
      continue;
    }
    if (state.recentlySoldQuestions.has(q.toLowerCase())) {
      skipSold++;
      continue;
    }
    ...
    if (yp >= PRICE_SWEET_SPOT_MIN && yp <= PRICE_SWEET_SPOT_MAX) {
      ...
      const distFrom40 = Math.abs(yp - 0.4);
      priceSweetSpot = Math.max(0, 1 - distFrom40 / 0.2);
    }
  ...
  if (scored.length === 0) {
    callbacks.log(
      `[INTEL:POLY] 0 markets passed (total: ${rawMarkets.length}, noYes: ${skipNoYes}, owned: ${skipOwned}, recent: ${skipRecent}, sold: ${skipSold}, pending: ${skipPending}, analyzed: ${skipAnalyzed}, price: ${skipPrice}, days: ${skipDays}, volume: ${skipVolume})`,
    );
  }
    ...
    for (let i = 0; i < topN.length; i++) {
      ...
      if (result.status === "fulfilled") {
        ...
        if (intel.isContrarian) {
          ...
          callbacks.log(
            `[INTEL:POLY] ⚠️ "${topN[i]!.question.slice(0, 50)}" — contrarian signal: ${intel.contrarian24hMove!.toFixed(0)}% 24h move`,
          );
        }

        if (intel.depth && !intel.depth.isLiquid) {
          callbacks.log(
            `[INTEL:POLY] ❌ "${topN[i]!.question.slice(0, 50)}" — illiquid ($${intel.depth.totalDepthUsd.toFixed(0)} depth), deprioritized`,
          );
          topN[i]!.score *= 0.3;
        }
      }
  ...
  for (const event of evData.data ?? []) {
    for (const m of (event.markets ?? []).filter(
      ...
      const score =
        ...
        midScore * SCORE_MIDPOINT_WEIGHT +
        volumeScore * SCORE_VOLUME_WEIGHT +
        jupTimeScore * SCORE_TIME_WEIGHT;
      ...
      if (
        eventTitle &&
        [...ownedTitles].some((t) => t.includes(eventTitle) || eventTitle.includes(t))
      ) {
        _jupDbgOwned++;
        continue;
      }
