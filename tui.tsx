...
function useFatalError(): string | null {
  ...
  useEffect(() => {
    ...
    return () => { fatalErrorCallbacks = fatalErrorCallbacks.filter((cb) => cb !== callback); };
  }, []);
  return error;
}
...
import { FatalErrorDisplay, ChatPanel, SidebarPanel } from "./tui-panels";
...
function getAutonomyStatus(runtime: AgentRuntime): { enabled: boolean; running: boolean; interval: number } | null {
  ...
  return { enabled: status.enabled ?? false, running: status.running ?? false, interval: status.interval ?? 5000 };
}
...
function PolymarketTuiApp(props: TuiSession): ReactNode {
  ...
  const handleInputChange = useCallback((value: string) => {
    if (hasMouseSequence(value)) { setInput((prev) => stripInputArtifacts(prev)); return; }
  ...
  useEffect(() => {
    ...
    const fetchBalance = async () => {
      ...
      if (!service && typeof runtime.getServiceLoadPromise === "function") {
        try { service = await runtime.getServiceLoadPromise(POLYMARKET_SERVICE_NAME) as PolymarketService; } catch {}
      }
      if (!service) {
  ...
  // Focus alignment
  useEffect(() => {
    if (layout === "chat") { setFocusPanel("chat"); return; }
  ...
  useEffect(() => {
    ...
    if (props.startupInfo?.length) {
      appendMessage({ id: uuidv4(), role: "system", content: props.startupInfo.join("\n"), timestamp: Date.now() });
    }
  ...
  useEffect(() => {
    if (!stdout) return;
    const update = () => setTerminalSize({ columns: stdout.columns ?? 100, rows: stdout.rows ?? 28 });
  ...
  useEffect(() => {
    ...
    return () => { stdout.write("\x1b[?1000l\x1b[?1006l\x1b[?1015l\x1b[?1007l"); };
  }, [stdout]);
  ...
  useEffect(() => {
    ...
    const onData = (data: Buffer) => {
      ...
      if (focusPanel === "chat") {
        ...
        setSidebarScrollOffset((prev) => Math.max(0, Math.min(sidebarMaxScroll, prev + scroll.delta)));
      }
    };
  ...
  useEffect(() => { setSidebarScrollOffset(0); }, [sidebarView]);

  // Sidebar data fetch
  useEffect(() => {
    if (sidebarView === "logs") { setSidebarLoading(false); return; }
    let isActive = true;
    const update = async () => {
      ...
      if (!service) {
        if (isActive) { setSidebarLoading(false); setSidebarContent("Polymarket service failed to start."); setSidebarUpdatedAt(formatTimestamp(new Date())); }
        return;
      }
      try {
        if (sidebarView === "positions") {
          ...
          const lines: string[] = [];
          const funderSetting = runtime.getSetting("POLYMARKET_FUNDER_ADDRESS") || runtime.getSetting("POLYMARKET_FUNDER") || runtime.getSetting("CLOB_FUNDER_ADDRESS");
          const funderAddress = normalizeSetting(funderSetting);
          const walletAddress = state?.walletAddress ?? "unknown";
          lines.push(`Account: ${funderAddress ? `Proxy ${shortenId(funderAddress)}` : `EOA ${shortenId(walletAddress)}`}`);
          const balance = state?.balances?.collateral?.balance;
          if (balance !== undefined) { setBalanceText(`USDC: $${balance}`); lines.push(`USDC: $${balance}`); }
          else lines.push("USDC: Unable to fetch");
          lines.push("");
          if (positions.length === 0) { lines.push("No positions found."); }
          else {
            lines.push(`Positions (${positions.length}):`);
            const entries = await Promise.all(positions.slice(0, 10).map(async (pos: { size: string; average_price: string; market?: string }, idx: number) => {
              ...
              if (marketId.startsWith("0x")) {
                ...
                else { try { const m = (await service.getClobClient().getMarket(marketId)) as Market; if (m?.question) { marketName = m.question; marketNameCacheRef.current.set(marketId, m.question); } } catch {} }
              }
              return `${idx + 1}. ${marketName}\n   ${side} ${Math.abs(size).toFixed(4)} @ ${odds}`;
            }));
            lines.push(...entries);
          }
          if (isActive) { setSidebarLoading(false); setSidebarContent(lines.join("\n")); setSidebarUpdatedAt(formatTimestamp(new Date())); }
        } else if (sidebarView === "markets") {
          interface MarketItem { id: string; title: string; volume: number; endDate: string | null; source: "gamma" | "clob"; }
          const gammaPromise = fetch("https://gamma-api.polymarket.com/events?closed=false&active=true&limit=20&order=volume&ascending=false").then(async (res) => {
            if (!res.ok) return [] as MarketItem[];
            interface GammaEvent { id?: string; slug?: string; title?: string; question?: string; endDate?: string; volume?: number; closed?: boolean; active?: boolean; }
            return ((await res.json()) as GammaEvent[]).filter((e) => e.active !== false && e.closed !== true).map((e): MarketItem => ({ id: e.id || e.slug || "", title: e.title || e.question || e.slug || "Unknown", volume: e.volume ?? 0, endDate: e.endDate || null, source: "gamma" }));
          }).catch(() => [] as MarketItem[]);
          const clobPromise = (async () => {
            ...
            const now = Date.now();
            interface ClobMarket { condition_id: string; question?: string; active?: boolean; closed?: boolean; end_date_iso?: string; }
            return (response?.data ?? []).filter((m: ClobMarket) => m.active && !m.closed && (!m.end_date_iso || new Date(m.end_date_iso).getTime() >= now)).map((m: ClobMarket): MarketItem => ({ id: m.condition_id, title: m.question || m.condition_id, volume: 0, endDate: m.end_date_iso || null, source: "clob" }));
          })().catch(() => [] as MarketItem[]);
          const [gammaMarkets, clobMarkets] = await Promise.all([gammaPromise, clobPromise]);
          const seen = new Set<string>();
          const combined: MarketItem[] = [];
          for (const m of gammaMarkets) { const key = m.title.toLowerCase().slice(0, 50); if (!seen.has(key)) { seen.add(key); combined.push(m); } }
          for (const m of clobMarkets) { const key = m.title.toLowerCase().slice(0, 50); if (!seen.has(key)) { seen.add(key); combined.push(m); } }
          combined.sort((a, b) => (b.volume - a.volume) || (a.endDate && b.endDate ? new Date(a.endDate).getTime() - new Date(b.endDate).getTime() : 0));
          ...
          const content = trimmed.length === 0 ? "No active markets found." : trimmed.map((m) => {
            const lines: string[] = [];
            ...
            lines.push(m.source === "gamma" ? `https://polymarket.com/event/${m.id}` : `https://polymarket.com/market/${m.id}`);
            return buildSidebarCard(m.title, lines, cardInnerWidth);
          }).join("\n\n");
          if (isActive) { setSidebarLoading(false); setSidebarContent(content); setSidebarUpdatedAt(formatTimestamp(new Date())); }
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (isActive) { setSidebarLoading(false); setSidebarContent(`Error: ${message}`); setSidebarUpdatedAt(formatTimestamp(new Date())); }
      }
    };
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      pollAutonomyLogs(runtime, lastAutonomyRef.current, (text) => {
        ...
        const now = Date.now();
        lines.forEach((line) => { appendMessage({ id: uuidv4(), role: "system", content: line, timestamp: now }); appendLog(line); });
      }).catch(() => undefined);
    }, 1500);
  ...
  useEffect(() => {
    ...
    const wrap = (level: "info" | "warn" | "error" | "debug", original?: LoggerMethod) => (...args: LogArg[]) => {
    ...
    const originalInfo = logger.info; const originalWarn = logger.warn; const originalError = logger.error; const originalDebug = logger.debug;
    ...
    return () => { if (originalInfo) logger.info = originalInfo; if (originalWarn) logger.warn = originalWarn; if (originalError) logger.error = originalError; if (originalDebug) logger.debug = originalDebug; };
  }, [appendLog, runtime]);
  ...
  useEffect(() => {
    const onActionStarted = (payload: unknown) => {
      ...
      const actionId = typeof content.actionId === "string" ? content.actionId : `${actionName}:${Date.now()}`;
      const messageId = uuidv4();
      actionMessageIdsRef.current.set(actionId, messageId);
      appendMessage({ id: messageId, role: "system", content: `calling ${actionName}...`, timestamp: Date.now() });
    ...
    const onActionCompleted = (payload: unknown) => {
      ...
      if (!content) return;
      const actionName = content.actions?.[0] ?? "action";
      const actionId = typeof content.actionId === "string" ? content.actionId : `${actionName}:done`;
      ...
      if (messageId) { updateMessage(messageId, `action ${actionName} ${status}`); actionMessageIdsRef.current.delete(actionId); }
      else { appendMessage({ id: uuidv4(), role: "system", content: `action ${actionName} ${status}`, timestamp: Date.now() }); }
    ...
    return () => { runtime.off(EventType.ACTION_STARTED, onActionStarted as never); runtime.off(EventType.ACTION_COMPLETED, onActionCompleted as never); };
  }, [appendLog, appendMessage, updateMessage, runtime]);

  // Autonomy status polling
  useEffect(() => {
    const check = () => { const status = getAutonomyStatus(runtime); if (status) setAutonomyEnabled(status.running); };
    check();
  ...
  const handleSubmit = useCallback(async (value: string) => {
    ...
    try {
      ...
      if (trimmed === "/help") {
        appendMessage({ id: uuidv4(), role: "system", content: ["Commands:", "  /autonomy [true|false] - Show status or toggle autonomous mode", "  /think - Trigger autonomous thinking immediately", "  /interval <seconds> - Set autonomy loop interval (5-600s)", "  /account - Show wallet and positions", "  /markets - Show active markets", "  /logs - Show agent logs", "  /error - Show recent errors", "  /clear - Clear chat history", "  /help - Show this help", "  /exit - Exit the application"].join("\n"), timestamp: Date.now() });
        return;
      }
      if (trimmed === "/error") {
        const errorLogs = logs.filter((log) => log.includes("ERROR") || log.includes("Error") || log.includes("error")).slice(-10);
        appendMessage({ id: uuidv4(), role: "system", content: errorLogs.length === 0 ? "No recent errors found. Check polymarket-error.log for crash logs." : `Recent errors (${errorLogs.length}):\n${errorLogs.join("\n")}`, timestamp: Date.now() });
        return;
      }
      if (trimmed === "/clear") { setMessages([]); return; }
      if (trimmed === "/account") { setSidebarView("positions"); setLayout("split"); return; }
      if (trimmed === "/markets") { setSidebarView("markets"); setLayout("split"); return; }
      if (trimmed === "/logs") { setSidebarView("logs"); setLayout("split"); return; }
      if (trimmed.startsWith("/autonomy")) {
        const parts = trimmed.split(/\s+/);
        const valueArg = parts[1];
        if (!valueArg) {
          const currentStatus = getAutonomyStatus(runtime);
          if (!currentStatus) appendMessage({ id: uuidv4(), role: "system", content: "Autonomy service not available.", timestamp: Date.now() });
          else appendMessage({ id: uuidv4(), role: "system", content: `Autonomy status: ${currentStatus.enabled ? "enabled" : "disabled"}, running: ${currentStatus.running ? "yes" : "no"}, interval: ${Math.round(currentStatus.interval / 1000)}s`, timestamp: Date.now() });
          return;
        }
        if (valueArg !== "true" && valueArg !== "false") { appendMessage({ id: uuidv4(), role: "system", content: "Usage: /autonomy [true|false]", timestamp: Date.now() }); return; }
      ...
      if (trimmed === "/think") {
        const svc = runtime.getService<AutonomyService>("AUTONOMY");
        if (!svc) { appendMessage({ id: uuidv4(), role: "system", content: "Autonomy service not available.", timestamp: Date.now() }); return; }
        if (svc.getStatus?.()?.thinking) { appendMessage({ id: uuidv4(), role: "system", content: "Already thinking... please wait.", timestamp: Date.now() }); return; }
        if (typeof svc.triggerThinkNow === "function") {
          appendMessage({ id: uuidv4(), role: "system", content: "Triggering autonomous thinking...", timestamp: Date.now() });
          appendLog("[Autonomy] Manual think triggered");
          try { const success = await svc.triggerThinkNow(); if (success === false) appendMessage({ id: uuidv4(), role: "system", content: "Think cycle skipped (already in progress or error occurred).", timestamp: Date.now() }); }
          catch (err) { const msg = err instanceof Error ? err.message : String(err); appendMessage({ id: uuidv4(), role: "system", content: `Think error: ${msg}`, timestamp: Date.now() }); appendLog(`[Autonomy] Think error: ${msg}`); }
          return;
        }
        const autonomousRoomId = svc.getAutonomousRoomId?.();
        if (!autonomousRoomId) { appendMessage({ id: uuidv4(), role: "system", content: "Could not get autonomous room ID.", timestamp: Date.now() }); return; }
        const thinkMsgId = uuidv4();
        appendMessage({ id: thinkMsgId, role: "system", content: "Triggering autonomous thinking...", timestamp: Date.now() });
        ...
        try {
          ...
          const autonomousMessage = createMessageMemory({ id: uuidv4() as UUID, entityId: userId, roomId, content: { text: autonomousPrompt, source: "manual-trigger", channelType: ChannelType.DM, metadata: { type: "autonomous-prompt", isAutonomous: true, isManualTrigger: true } } });
          const result = await messageService.handleMessage(runtime, autonomousMessage, async (content: Content) => {
            if (typeof content.text === "string" && content.text.trim()) { appendMessage({ id: uuidv4(), role: "assistant", content: `[Autonomous] ${content.text}`, timestamp: Date.now() }); appendLog(`[Autonomy] Response: ${content.text.slice(0, 100)}...`); }
            return [];
          });
          updateMessage(thinkMsgId, `Autonomous thinking complete. Responded: ${result.didRespond ? "yes" : "no"}`);
        } catch (err) { const msg = err instanceof Error ? err.message : String(err); updateMessage(thinkMsgId, `Think error: ${msg}`); appendLog(`[Autonomy] Think error: ${msg}`); }
        return;
      }
      if (trimmed.startsWith("/interval")) {
        ...
        if (!valueArg) {
          const currentStatus = getAutonomyStatus(runtime);
          appendMessage({ id: uuidv4(), role: "system", content: `Current autonomy interval: ${currentStatus ? Math.round(currentStatus.interval / 1000) : "unknown"}s. Usage: /interval <seconds> (5-600)`, timestamp: Date.now() });
          return;
        }
        const seconds = Number.parseInt(valueArg, 10);
        if (Number.isNaN(seconds) || seconds < 5 || seconds > 600) { appendMessage({ id: uuidv4(), role: "system", content: "Invalid interval. Must be between 5 and 600 seconds.", timestamp: Date.now() }); return; }
        const svc = runtime.getService<AutonomyService>("AUTONOMY");
        if (!svc || typeof svc.setLoopInterval !== "function") { appendMessage({ id: uuidv4(), role: "system", content: "Autonomy service not available or setLoopInterval not supported.", timestamp: Date.now() }); return; }
        svc.setLoopInterval(seconds * 1000);
        appendMessage({ id: uuidv4(), role: "system", content: `Autonomy interval set to ${seconds} seconds.`, timestamp: Date.now() });
      ...
      appendMessage({ id: assistantId, role: "assistant", content: "(processing...)", timestamp: Date.now() });
      appendLog("🔄 Processing...");

      const message = createMessageMemory({ id: uuidv4() as UUID, entityId: userId, roomId, content: { text: trimmed, source: "polymarket-demo", channelType: ChannelType.DM } });
      let callbackText = "";
      const actionResultIds: string[] = [];

      await messageService.handleMessage(runtime, message, async (content: Content) => {
        if (typeof content.text === "string" && content.text.trim()) {
          const text = content.text.trim();
          const isActionResult = text.startsWith("⏳") || text.startsWith("🔍") || text.startsWith("📊") || text.startsWith("❌") || text.startsWith("✅") || text.includes("**");
          if (isActionResult) {
            const resultId = uuidv4();
            actionResultIds.push(resultId);
            appendMessage({ id: resultId, role: "assistant", content: text, timestamp: Date.now() });
            appendLog(`Action Result: ${text.slice(0, 100)}...`);
          } else { callbackText = text; }
        }
        return [];
      }, {} as never);

      const finalText = callbackText.trim();
      if (!finalText) { updateMessage(assistantId, "(no response)"); appendLog("Eliza: (no response)"); }
      else { updateMessage(assistantId, finalText); appendLog(`Eliza: ${finalText}`); }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      appendMessage({ id: uuidv4(), role: "system", content: `❌ Error: ${errorMessage}`, timestamp: Date.now() });
      appendLog(`ERROR: ${errorMessage}`);
      if (error instanceof Error && error.stack) appendLog(`Stack: ${error.stack}`);
      if (errorMessage.includes("FATAL") || errorMessage.includes("Cannot read") || errorMessage.includes("undefined is not") || errorMessage.includes("null is not")) throw error;
  ...
  useInput((inputVal, rawKey) => {
    ...
    if (key.ctrl && key.name === "c") {
      if (messages.length > 0) { setInput(""); setScrollOffset(0); setMessages([]); return; }
    ...
    if ((key.shift && key.tab) || inputVal === "\x1b[Z") {
      if (layout === "split" || layout === "chat") { setLayout("chat"); setFocusPanel("chat"); }
      else { setLayout(isWide ? "split" : "chat"); setFocusPanel("chat"); }
      return;
    }
    if (key.tab && !key.ctrl && !key.meta) {
      if (layout === "split") setFocusPanel((prev) => (prev === "chat" ? "sidebar" : "chat"));
      else if (layout === "chat") { setLayout(isWide ? "split" : "sidebar"); setFocusPanel("sidebar"); }
      else { setLayout(isWide ? "split" : "chat"); setFocusPanel("chat"); }
      return;
    }
    if (key.return && focusPanel === "sidebar") { cycleSidebarView(); return; }
    if (focusPanel === "chat") {
      if (key.pageUp) { setScrollOffset((prev) => Math.min(chatMaxScroll, prev + 10)); return; }
      if (key.pageDown) { setScrollOffset((prev) => Math.max(0, prev - 10)); return; }
      if (key.upArrow || key.downArrow) { setScrollOffset((prev) => Math.max(0, Math.min(chatMaxScroll, prev + (key.upArrow ? 1 : -1)))); return; }
    }
    if (focusPanel === "sidebar") {
      if (key.pageUp) { setSidebarScrollOffset((prev) => Math.min(sidebarMaxScroll, prev + 10)); return; }
      if (key.pageDown) { setSidebarScrollOffset((prev) => Math.max(0, prev - 10)); return; }
      if (key.upArrow || key.downArrow) { setSidebarScrollOffset((prev) => Math.max(0, Math.min(sidebarMaxScroll, prev + (key.upArrow ? 1 : -1)))); return; }
    }
  });
    ...
    <Box flexDirection="column" width={columns} height={rows}>
      ...
      <Box flexDirection="row" gap={gap} height={bodyHeight} overflow="hidden">
        <Box display={showChatPanel ? "flex" : "none"} width={chatWidth} height={bodyHeight}>
          <ChatPanel messages={messages} input={input} onInputChange={handleInputChange} onSubmit={handleSubmit} width={chatWidth} height={bodyHeight} scrollOffset={scrollOffset} onMaxScrollChange={setChatMaxScroll} isActive={focusPanel === "chat"} />
        </Box>
        <Box display={showSidebarPanel ? "flex" : "none"} width={isWide ? sidebarWidth : chatWidth} height={bodyHeight}>
          <SidebarPanel view={sidebarView} content={sidebarContent} loading={sidebarLoading} updatedAt={sidebarUpdatedAt} width={isWide ? sidebarWidth : chatWidth} height={bodyHeight} logs={logs} scrollOffset={sidebarScrollOffset} onMaxScrollChange={setSidebarMaxScroll} isActive={focusPanel === "sidebar"} />
        </Box>
      </Box>
    </Box>
  );
...
export async function runPolymarketTui(session: TuiSession): Promise<void> {
  ...
  try {
    ...
    if (instance) { try { instance.unmount(); } catch {} }
    throw error;
  } finally {
